...
model_vars <- c("cardiometabolic_diseases", "CHG", "Age", "Gender", "Education", 
...
# 构建逻辑回归模型
fit <- lrm(cardiometabolic_diseases ~ rcs(CHG, 4) + 
...
# 提取P值
chg_p_overall <- anova_results["CHG", "P"]
chg_p_nonlinear <- anova_results[" Nonlinear", "P"]
...
p_overall_text <- format_p_value(chg_p_overall)
p_nonlinear_text <- format_p_value(chg_p_nonlinear)

# 获取RCS节点位置
knots_positions <- attr(rcs(model_data$CHG, 4), "parms")
cat("RCS节点位置 (knots):", knots_positions, "\n")

# 生成预测值
OR <- Predict(fit, CHG, ref.zero = TRUE, fun = exp)

# 确定参考值位置 (通常是最小值或中位数)
reference_value <- min(OR$CHG)
reference_index <- which.min(OR$CHG)

# 创建P值标签数据框
p_label_data <- data.frame(
  x = min(OR$CHG) + (max(OR$CHG) - min(OR$CHG)) * 0.05,
  y = max(OR$upper) * 0.95,
  label = paste0("P-overall = ", p_overall_text, "\nP-non-linear = ", p_nonlinear_text)
)

# 创建图形对象并赋值给变量p
p <- ggplot(OR, aes(x = CHG, y = yhat)) + 
  # 添加参考线
  ...
  # 添加节点信息标注（可选）
  annotate("text",
           x = min(OR$CHG) + (max(OR$CHG) - min(OR$CHG)) * 0.65,
           y = max(OR$upper) * 0.7,
  ...
  labs(
    x = "CHG Index", 
    y = "Odds Ratio (95% CI)",
    title = "Association Between CHG Index and Cardiometabolic Diseases",
...
# 保存为高分辨率PDF
ggsave("CHG_RCS_Plot.pdf", plot = p, 
       width = 12, height = 12, device = cairo_pdf)  # 增加图像尺寸

# 保存为高分辨率TIFF
ggsave("CHG_RCS_Plot.tiff", plot = p, 
