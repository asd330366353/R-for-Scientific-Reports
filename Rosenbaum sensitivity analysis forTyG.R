...
# 查看TyG变量的分布并创建二分类变量
summary(data$TyG)
median_tyg <- median(data$TyG, na.rm = TRUE)
data$TyG_binary <- ifelse(data$TyG > median_tyg, 1, 0)
...
data$TyG_binary <- as.factor(data$TyG_binary)

# 构建倾向得分模型并进行匹配
ps_model <- glm(TyG_binary ~ Age + Gender + Education + Smoking + Drinking + 
                  Residence + Married_status + Sleep + BMI,
                family = binomial(), data = data)

matched_data <- matchit(TyG_binary ~ Age + Gender + Education + Smoking + Drinking + 
                          Residence + Married_status + Sleep + BMI,
...
# 将因子转换为数值
matched_df$TyG_binary_num <- as.numeric(as.character(matched_df$TyG_binary))
matched_df$cardiometabolic_diseases_num <- as.numeric(as.character(matched_df$cardiometabolic_diseases))

# 进行 McNemar 检验
mcnemar_table <- table(matched_df$TyG_binary_num, matched_df$cardiometabolic_diseases_num)
print("McNemar 检验表:")
print(mcnemar_table)
...
p <- ggplot(sensitivity_df, aes(x = Gamma, y = P_value)) +
  ...
  labs(title = "Rosenbaum Sensitivity Analysis for TyG Effect on Cardiometabolic Diseases",
...
# 保存图形
ggsave("rosenbaum_sensitivity_analysis_TyG.png", plot = p, width = 10, height = 8, dpi = 300)
